...
class TAssociationRepository
{
    ...
    private function updateAssociations($ownerId, $newValues=[],$itemsTable=null, $filterTable=null) {
        if (empty($itemsTable)) {
            throw new \Exception('Items table not set');
        }
        if (empty($filterTable)) {
            throw new \Exception('Filter table not set');
        }
