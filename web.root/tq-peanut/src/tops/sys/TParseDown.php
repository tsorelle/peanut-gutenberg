<?php
/************
 * Adapted by Terry SoRelle for Tops
...
class TParseDown
{
    ...
    const version = '1.8.0-beta-7-tls';
     ...
     * @return mixed
     */
    public static function toHtml(string $mdText) {
     ...
     * @return void
     */
    function setNewWindowForExternalUrl($yes = true) {
        $this->newWindowForExternalUrl = $yes;
    }
    protected $newWindowForExternalUrl = true;

    // Modified setter for default value - TLS
    function setBreaksEnabled($breaksEnabled=true)
    {
    ...
    // Modified setter for default value - TLS
    function setMarkupEscaped($markupEscaped = true)
    {
    ...
    // Modified setter for default value - TLS
    function setUrlsLinked($urlsLinked = true)
    {
    ...
    // Modified setter for default value - TLS
    function setSafeMode($safeMode = true)
    {
    ...
    // Modified setter for default value - TLS
    function setStrictMode($strictMode = true)
    {
    ...
    private function parseLinks(string $s)
    {
        $p2 = strpos($s, '](');
        while (($p2 = strpos($s, '](') !== false)) {
/*            if ($p2 === false) {
                return $s;
            }*/
            $p1 = strrpos($s, '[');
            $p3 = strrpos($s, ')', $p2);
            $url = substr($s, $p2 + 2, $p3 - $p2 - 2);
            $ext = strpos($url, 'http');
            $format = ($this->newWindowForExternalUrl && (strpos($url, 'http') === 0)) ?
